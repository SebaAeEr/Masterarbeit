...
def CollectOPData(i, tabs):
    ...
    return max(transTimes(blockedwall), wall_time), cpu_time, spilltemp
...
def TPC():
    ...
    fig, ax = plt.subplots(1, 2)
    ax31 = ax[0]
    ax31.scatter(all_hash_times, all_hash_spills, marker=".", label="Hash")
    ax31.scatter(all_join_times, all_join_spills, marker="s", label="Join")
    ax31.scatter(all_agg_times, all_agg_spills, marker="^", label="Aggregation")
    ax31.legend(loc="upper right")
    ax31.set_ylabel("Wall time of Operation in min")
    ax31.set_xlabel("Data spilled by Operation in GB")

    ax32 = ax[1]
    ax32.scatter(all_hash_times_cpu, all_hash_spills, marker=".", label="Hash")
    ax32.scatter(all_join_times_cpu, all_join_spills, marker="s", label="Join")
    ax32.scatter(all_agg_times_cpu, all_agg_spills, marker="^", label="Aggregation")
    ax32.legend(loc="upper right")
    ax32.set_ylabel("CPU time of Operation in min")
    ax32.set_xlabel("Data spilled by Operation in GB")
